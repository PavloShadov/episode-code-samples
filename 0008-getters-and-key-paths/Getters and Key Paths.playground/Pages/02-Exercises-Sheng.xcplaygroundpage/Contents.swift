...
func reduce<R, A>(_ initialValue: R, _ nextPartialResult: @escaping (R, A) -> R) -> ([A]) -> R {
    return { $0.reduce(initialValue, nextPartialResult) }
}

func combining<Root, Value>(
    _ get: @escaping (Root) -> Value,
...
let podcasts = [
    Podcast(title: "Moviebuff",
            episodes: [
                Episode(topic: "Star Wars", isFinished: false),
                Episode(topic: "Star Trek", isFinished: false),
                Episode(topic: "Up", isFinished: true)
    ]),
    Podcast(title: "TripAdvisor",
            episodes: [
                Episode(topic: "Japan", isFinished: true),
                Episode(topic: "Spain", isFinished: false),
                Episode(topic: "America", isFinished: true)
    ])
]

extension Bool {
    var toInteger: Int { self ? 1 : 0 }
}

// A podcast is on track means that there are more than half episodes are finished
func isOnTrack(_ total: Int) -> (Int) -> String {
    return { finished in
        if finished * 2 >= total {
            return "👍"
        } else {
...
func showOnTrack(for podcast: Podcast) -> String {
    let checkOnTrack = ^\Podcast.episodes.count >>> isOnTrack
    let countFinishedEpisodes = ^\Podcast.episodes >>> reduce(0, combining(^\Episode.isFinished.toInteger, by: +))
    let checkOnTrackForPodcast = podcast |> checkOnTrack
    
    return podcast |> countFinishedEpisodes |> checkOnTrackForPodcast
}

// [Podcast] -> [String] 👍 or 👎
podcasts |> map(showOnTrack(for:))
