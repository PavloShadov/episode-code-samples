...
func to<A>(_ f: @escaping (A) -> Void) -> A {
    fatalError("???")
}

func from<A>(_ a: A) -> (A) -> Void {
    return {a in
        // ???
    }
}
...
func to(_ f: @escaping (Never) -> Never) -> Void {
    // f(???)
}
...
func to<A>(_ f: @escaping (A) -> Never) -> Never {
    fatalError("Should do nothing?")
}
